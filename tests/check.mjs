...
for (const [input, expected] of [[-1,4],[0,0],[1,1],[2,2],[3,3],[4,4],[5,0],[7,2],[-7,3]]) assert.equal(wrapIndex(input), expected);
...
assert.deepEqual(projects.map(project => new URL(project.url).hostname), ['andys.rest','emcotec.ru','www.artcom.ru','cleanroomshop.ru','github.com']);
...
assert.equal((html.match(/class="page-marker/g)||[]).length,projects.length);
assert(html.includes(`<span>/ ${String(projects.length).padStart(2,'0')}</span>`));
console.log('PASS: carousel boundaries, five projects, HTTPS links, owner contacts, unique IDs, anchors, assets, accessibility hooks, no data collection');
