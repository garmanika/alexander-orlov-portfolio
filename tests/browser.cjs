...
(async () => {
  ...
  assert.equal(await page.locator('.project-card > picture > img').count(),5);
  ...
  await page.locator('#next').click();
  assert.equal(await page.locator('#project-title').textContent(),'Зерновая 52');
  await page.locator('#next').click();
  assert.equal(await page.locator('#project-title').textContent(),"Andy's");
  ...
  assert.equal(await page.locator('#detail-link').getAttribute('href'),'https://github.com/garmanika/grain-52');
